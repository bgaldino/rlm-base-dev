...
3. Every tracked plan needs a **README** — a new plan without one, or an existing plan whose behavior or objects changed without a README update, both fail `python scripts/ai/check_plan_readme_consistency.py <plan_dir>` (repo-wide with no argument): a missing README is a named **error**, and an existing one fails if its object table or `# N records` listings drift from the actual `export.json`/CSVs (record counts, operations, externalIds, phantom/missing objects). `scripts/ai/generate_plan_readme.py <plan_dir>` derives a minimal, mechanically-accurate README from `export.json` + CSVs for a plan that has none — regenerate it rather than hand-writing the table after a real change. Must report **0 errors**.
