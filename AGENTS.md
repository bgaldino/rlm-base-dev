...
| `decision-tables/authoring-and-data-model.md` | Decision Tables | The 5 Tooling setup objects + ID prefixes + the `Metadata` complexvalue; annotated `.decisionTable-meta.xml`; Metadata deploy + Tooling authoring and the read-only Connect vocabulary; full enum catalog; the definition-vs-data two-layer model in depth |
