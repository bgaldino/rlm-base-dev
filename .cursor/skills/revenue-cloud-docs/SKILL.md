...
> #   (either case now raises — but only if expect_min_articles is set for this area)
> ```
>
...
| `cumulusci.yml` | Task registrations: `snapshot_salesforce_help` (generic), plus per-release area variants — the `snapshot_*_262` family, `snapshot_billing_help_260`, and the `snapshot_*_264` family. Four 264 areas (`configurator`, `transaction_mgmt`, `billing`, `pcm`) are captured and root-verified; the other nine inherited their root article IDs from 262 **unverified** — run `-o mode discover` first |
