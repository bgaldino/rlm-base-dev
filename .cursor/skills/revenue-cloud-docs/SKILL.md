...
| `cumulusci.yml` | Task registrations: `snapshot_salesforce_help` (generic), plus per-release area variants — the `snapshot_*_262` family, `snapshot_billing_help_260`, and the `snapshot_*_264` family. Six 264 areas (`configurator`, `transaction_mgmt`, `billing`, `pcm`, `dro`, `pricing`) are captured and root-verified; the other five `snapshot_*_help_264` tasks inherited their root article IDs from 262 **unverified** — run `-o mode discover` first. (`snapshot_dev_guide_264`/`snapshot_industries_dev_guide_264` are a separate task family with no `root_article_id` — already captured, see the branch note above.) |
