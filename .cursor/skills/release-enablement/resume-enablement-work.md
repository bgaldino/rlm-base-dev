...
1. **DO NOT push to `main` or the active release branch (`264`) directly.** Enablement work always uses a feature branch plus a PR. Per `AGENTS.md` rule 8: never push or force-push either without explicit user approval — PRs are routinely stacked on the release branch, so rewriting it invalidates every one of them.
