...
Full Manufacturing feature setup: deploys core metadata (assets, theme, fields, Apex), assigns Manufacturing permission sets and groups (with propagation sleep), deploys all business process flows and Quick Actions, context plan extensions, document generation (ODTs and document templates), seed data (PCM, pricing, DRO, CML, tax, billing), guided selling, rebates, advanced account forecasting, UX assembly, and theme activation. Gated by manufacturing=true in prepare_rlm_org (step 33).
...
4. **flow** `prepare_mfg_context_plan`
5. **flow** `prepare_mfg_docgen`  `when: project_config.project__custom__manufacturing and project_config.project__custom__mfg_docgen`
6. **flow** `prepare_mfg_data`
7. **task** `update_product_fulfillment_decomp_rules`  `when: project_config.project__custom__dro and project_config.project__custom__manufacturing`
8. **flow** `prepare_mfg_visuals`
9. **flow** `prepare_mfg_guided_selling`
10. **flow** `prepare_mfg_rebates`
11. **flow** `prepare_mfg_aaf`
12. **task** `util_sleep`  `when: project_config.project__custom__manufacturing`
   - `seconds`: `30`
13. **flow** `prepare_mfg_ux`
14. **task** `activate_mfg_theme`  `when: project_config.project__custom__manufacturing`
...
5. **task** `deploy_billing_id_settings_mfg`  `when: project_config.project__custom__billing`
6. **task** `deploy_post_billing`  `when: project_config.project__custom__billing`
7. **task** `deploy_billing_template_settings`  `when: project_config.project__custom__billing`

---

### `prepare_mfg_context_plan`

Apply Manufacturing context attributes to the standard RLM_SalesTransactionContext definition using the MFG manifest. Runs the apply step twice (two-pass pattern required for full attribute population).

**Steps:**

1. **task** `apply_mfg_SalesTransactionContext`  `when: project_config.project__custom__manufacturing`
...
Deploy Manufacturing document generation components: OmniDataTransforms (Quote/Contract extraction and mapping), and document templates (RLM_MFG_Sales_Proposal_Document, Price_Contract). After deploying templates, activates them (step 2) and fixes the Salesforce batch-deploy binary bug that causes all templates to receive the same ContentDocument binary (step 3). Gated by manufacturing=true and mfg_docgen=true.
...
2. **task** `deploy_mfg_doc_templates`  `when: project_config.project__custom__manufacturing and project_config.project__custom__mfg_docgen`
3. **task** `activate_mfg_docgen_templates`  `when: project_config.project__custom__manufacturing and project_config.project__custom__mfg_docgen`
4. **task** `fix_mfg_document_template_binaries`  `when: project_config.project__custom__manufacturing and project_config.project__custom__mfg_docgen`
...
9. **task** `deploy_billing_id_settings`  `when: project_config.project__custom__billing and not project_config.project__custom__manufacturing`
...
33. **flow** `prepare_manufacturing`  `when: project_config.project__custom__manufacturing`
34. **flow** `refresh_all_decision_tables`
35. **task** `rebuild_search_index`
36. **flow** `stamp_git_commit`
