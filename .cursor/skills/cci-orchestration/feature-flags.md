...
**48 feature flags**, **96 configuration values**, **44 YAML anchors** under `project.custom`.
...
| `tax` | `True` | 11 flow step(s) |
| `trial` | `False` | — |
| `tso` | `False` | 19 flow step(s) |
...
- `prepare_manufacturing` step 7 → `update_product_fulfillment_decomp_rules`
- `prepare_core` step 16 → `create_dro_rule_library`
...
### `manufacturing` (default: `False`)

- `prepare_rlm_org` step 32 → `prepare_manufacturing`
...
- `prepare_manufacturing` step 5 → `prepare_mfg_docgen`
- `prepare_manufacturing` step 7 → `update_product_fulfillment_decomp_rules`
- `prepare_manufacturing` step 12 → `util_sleep`
- `prepare_manufacturing` step 14 → `activate_mfg_theme`
- `prepare_billing` step 9 → `deploy_billing_id_settings`
...
- `prepare_mfg_docgen` step 2 → `deploy_mfg_doc_templates`
- `prepare_mfg_docgen` step 3 → `activate_mfg_docgen_templates`
- `prepare_mfg_docgen` step 4 → `fix_mfg_document_template_binaries`
...
- `prepare_mfg_data` step 6 → `prepare_mfg_billing`
- `prepare_mfg_context_plan` step 1 → `apply_mfg_SalesTransactionContext`
