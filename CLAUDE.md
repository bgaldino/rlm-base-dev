...
- **pyenv** via `brew install pyenv` is the recommended Python version manager; **Python 3.12 or 3.13 recommended for CCI** (3.14 has known dependency issues). Install with `pyenv install 3.13`.
- **pipx** is preferred for CumulusCI: install pipx via pyenv (`$(pyenv prefix 3.13)/bin/python3.13 -m pip install --user pipx`), NOT `brew install pipx` (Homebrew pipx uses its own bundled Python, not pyenv). Then: `pipx install cumulusci --python $(pyenv prefix 3.13)/bin/python3.13` and `pipx inject cumulusci "setuptools<71"`. The setuptools pin is required — CCI 4.x depends on `pyfilesystem2` which needs `pkg_resources` removed in setuptools 71+.
...
- Robot Framework deps (for headless tasks): `pipx inject cumulusci robotframework robotframework-seleniumlibrary webdriver-manager "urllib3>=2.6.3"`
