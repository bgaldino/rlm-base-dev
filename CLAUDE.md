...
- **Agent playbook:** `AGENTS.md` → **Customer Demo Product Onboarding UX** — the conductor contract (intake, vision gate, wave order, deploy gate). Onboarding runs as a **multi-agent flow**: `.cursor/skills/rlm-customer-demo-conductor/` orchestrates parallel domain builders (`rlm-customer-demo-pcm`, `-billing`, `-pricing`, `-usage-rates`, `-dro`, `-experience`) against a frozen **SKU contract** (`docs/references/customer-demo-sku-contract.md`), then runs `rlm-customer-demo-integrator` in series before any deploy. Domain pitfalls live in those skills and in file-scoped rules under `.cursor/rules/`, not in `AGENTS.md`.
- **Technical runbook:** `docs/guides/customer-demo-onboarding.md` — Product2 column alignment with **`qb-pcm`**, **`MissingParentRecordsReport.csv`** triage, verification CSV (`ProductTypeExpected` + **`CategoryCode`**), quote lines (**`SF-USG-*`** not **`SF-BLNG-*`**), **`AttributePicklistValue.Code`** global uniqueness, **`AttributeDefinition.DeveloperName`** prefixing, static resource + `DisplayUrl` sequencing.
- **Shared orgs:** demo orgs usually already hold other customers' catalogs. When they do, set `org.load_mode: additive` in the SKU contract and run **`prepare_customer_demo_catalog_additive`** — identical step order with `customer_demo_purge_records` and the four `delete_customer_demo_*` steps removed, so nothing existing is deleted. It is **not idempotent**: the seven `Insert`-operation objects (PUR, PURP, PUG, RateCardEntry, RABT, AAC, ABA) duplicate on a second run. Also prefer **`activate_customer_demo_rating_records`** over `activate_rating_records` there — the QB task activates every non-Active record org-wide and aborts on any pre-existing `UnitOfMeasureClass` whose default unit belongs to no class.
- **Flow:** `cci flow run prepare_customer_demo_catalog --org <alias>` — PCM first (products + **UnitOfMeasure / UnitOfMeasureClass** when metering is in scope); then deploy `unpackaged/post_customer_demo/staticresources`; with **`customer_demo_branding: true`**, deploy customer-branded LightningExperienceTheme (ContentAsset + BrandingSet + theme; **manual activation required** — Setup → Themes and Branding); product-images SFDMU; billing; `customer_demo_recreate_pricebook_via_api`; `customer_demo_verify_catalog`. With **`customer_demo_usage: true`**, append scoped **`delete_customer_demo_rates_data`** → **`delete_customer_demo_rating_data`** → rating load → rates load → **`activate_rating_records`** → **`activate_rates`** (do not load rating before PCM creates referenced SKUs and UoM rows). With **`customer_demo_dro: true`**, append **`delete_customer_demo_dro_data`** → **`insert_customer_demo_dro_data`** → **`update_customer_demo_fulfillment_decomp_rules`** (requires `prepare_dro` to have run first — QB DRO infrastructure must exist in the org).
- **Branding:** `prepare_customer_demo_branding` generates the theme from `--company-name`, `--logo-url`, `--brand-color` into `unpackaged/post_customer_demo/branding/`. Deployed by `deploy_customer_demo_branding` (step 4 when `customer_demo_branding: true`). Salesforce has no API for theme activation — **always manual**. Full design: `docs/features/customer-demo-branding.md` (platform constraints, image processing, naming conventions, activation workaround research).
- **Troubleshooting:** if pricebook recreation says **Missing Product2 for SKUs** after a “successful” PCM run, inspect **`datasets/sfdmu/customer-template/en-US/customer-template-pcm/reports/MissingParentRecordsReport.csv`** and fix **`Product2.csv`** (usually missing **`BasedOn.Code` / quantity UOM** columns — mirror **`qb-pcm/Product2.csv`**). **SFDMU v5 silent failures:** `ProductUsageGrant` Insert reports success but record never persists — always verify PUG after rating load; grant policies (`UsageGrantRenewalPolicy`, `UsageGrantRolloverPolicy`, `UsageOveragePolicy`) also silently fail to create — always reference **existing org policies** (e.g. `SF-DEMO-USG-RENEW`, `SF-DEMO-USG-ROLL`, `Default Usage Overage Policy`).
...
11. **Customer demo template** — `Product2.csv` columns must satisfy PCM `export.json` / SFDMU relationship resolution (see `docs/guides/customer-demo-onboarding.md`); pricebook CSV **`ProductTypeExpected`** when enforcing **Bundle vs blank `Type`**; **`CategoryCode`** must be populated (verify checks `ProductCategoryProduct` per SKU + category); **`AttributePicklistValue.Code`** must be globally unique across picklists (collisions cascade-fail entire attribute framework); **`AttributeDefinition.DeveloperName`** should be customer-prefixed to avoid org collisions; usage quote lines are **`SF-USG-*`**, not **`SF-BLNG-*`**.
12. **Customer demo branding** — `unpackaged/post_customer_demo/branding/` is generated output (ContentAsset + BrandingSet + LightningExperienceTheme); edit `scripts/customer-demo/prepare_customer_branding.py` for structural changes. BrandingSet API name must use `LEXTHEMING` prefix. Theme activation is **manual only** (no Salesforce API exists).
13. **Shared-org safety** — anything loaded by name or code (PaymentTerm, BillingPolicy, BillingTreatment, LegalEntity, catalog/classification codes, `AttributeDefinition.DeveloperName`, `AttributePicklistValue.Code`, UOM class/unit codes) Upserts on that key, so an unprefixed name **overwrites** an existing org record rather than creating one. Every such value must carry the customer prefix. Activation Apex must use an explicit allowlist, never "every Draft record" — and SOQL has no `NOT LIKE` operator (`WHERE (NOT Name LIKE 'QB%')` is the valid form; the invalid one fails to compile with a misleading "Unexpected token '<'"). Teardown script prefix lists are appended to, never replaced. See `.cursor/rules/customer-demo-apex.mdc`.
