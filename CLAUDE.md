...
unpackaged/post_tso_appmenu/ # App Launcher (AppSwitcher) order; captured by sync_appmenu_from_user.py; deployed when tso=true
...
- **Primary repo location:** `unpackaged/post_tso_appmenu/appMenus/AppSwitcher.appMenu-meta.xml` — captures the desired org-default App Launcher order. Never store org-specific emails or personally identifiable identifiers here.
- **UX assembly copy:** `templates/appMenus/base/AppSwitcher.appMenu-meta.xml` — used by `assemble_and_deploy_ux` when `tso=true`. **Must be kept in sync with `post_tso_appmenu` manually** — `sync_appmenu_from_user.py` only writes to `post_tso_appmenu/`. When updating the AppSwitcher order, copy the updated file to both locations.
- **Capture user order:** Run `python scripts/sync_appmenu_from_user.py` with the default org set to the org where the user has customized the App Launcher. Writes to `post_tso_appmenu/` only. No deploy. Then copy to `templates/appMenus/base/` to keep them in sync.
- **Deploy via prepare_tso:** `deploy_post_tso_app_menu` deploys `post_tso_appmenu/`; runs at step 5 of `prepare_tso` when `tso=true`.
- **Deploy via prepare_ux:** `assemble_and_deploy_ux` assembles `templates/appMenus/base/AppSwitcher.appMenu-meta.xml` into `unpackaged/post_ux/appMenus/` and deploys it when `tso=true`. This runs at step 29 (after `prepare_tso` at step 20), so **the `templates/appMenus/` version is the final deployed state** in a full `prepare_rlm_org` run.
...
# Also copy updated file to templates/appMenus/base/ to keep them in sync
python scripts/sync_appmenu_from_user.py
cp unpackaged/post_tso_appmenu/appMenus/AppSwitcher.appMenu-meta.xml templates/appMenus/base/
...
cci task run assemble_and_deploy_ux \
    --metadata-name RLM_Quote_Record_Page.flexipage-meta.xml --org dev-sb0

# Assemble only layouts (no deploy)
cci task run assemble_and_deploy_ux --metadata-type layouts --deploy false --org dev-sb0
...
9. **App Launcher** — when AppSwitcher is updated via `sync_appmenu_from_user.py`, both `post_tso_appmenu/` AND `templates/appMenus/base/` must be updated. `post_tso_appmenu` is deployed by `prepare_tso` (step 20); `templates/appMenus/base` is the final deployed state via `prepare_ux` (step 29).
