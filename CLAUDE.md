...
- **pyenv** via `brew install pyenv` is the recommended Python version manager; **Python 3.13 recommended for CCI** (3.14 has known dependency issues). Install the latest 3.13.x patch: `PYTHON_VERSION=$(pyenv install --list | grep -E "^[[:space:]]*3\.13\." | grep -v dev | tail -1 | tr -d '[:space:]') && pyenv install "$PYTHON_VERSION" && pyenv global "$PYTHON_VERSION"`.
- **pipx** is preferred for CumulusCI: install pipx via pyenv (`$(pyenv prefix)/bin/python3 -m pip install --user pipx`), NOT `brew install pipx` (Homebrew pipx uses its own bundled Python, not pyenv). Then: `pipx install cumulusci --python "$(pyenv prefix)/bin/python3"` and `pipx inject cumulusci "setuptools<71"`. Ensure `pyenv global` is set to 3.13 before running. The setuptools pin is required — CCI 4.x depends on `pyfilesystem2` which needs `pkg_resources` removed in setuptools 71+.
