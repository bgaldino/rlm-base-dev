...
### ProductSellingModelOption — One-Time selling model cannot have a ProrationPolicy

When creating a `ProductSellingModelOption` for a `OneTime` selling model, Salesforce rejects any non-null `ProrationPolicyId` with:

> **"A Proration Policy isn't allowed when the selling model type is One Time."**

In `ProductSellingModelOption.csv`, leave `ProrationPolicy.Name` **empty** for every row that references a `One-Time;OneTime` selling model. Only `TermDefined` and `Evergreen` selling models support proration policies.

### ProductSellingModelOption — SFDMU Bug 3 causes always-insert (expected duplicate errors)

The `externalId` for `ProductSellingModelOption` uses 1-hop relationship traversals (`Product2.StockKeepingUnit;ProductSellingModel.Name;ProductSellingModel.SellingModelType`). Per SFDMU Bug 3, Upsert with relationship-traversal externalIds always inserts. Re-running the PCM load will attempt to insert duplicate PSMOs; Salesforce rejects these with "A product selling model option already exists" errors. SFDMU reports `0 records failed` despite these errors — this is expected behavior. On subsequent runs, only net-new PSMOs (e.g. for newly added products) will actually be created; all others fail silently.

### ConfigureDuringSale — MUST be Allowed on ALL products with ProductAttributeDefinition rows

**Any product that has `ProductAttributeDefinition` records must have `ConfigureDuringSale = Allowed`.** Without it, the attribute configuration panel does not appear during the quote flow — the product cannot be configured and attribute-based pricing rules never fire.

Set `Allowed` on:
- **Every product with at least one PAD row** — even if the attribute is read-only or informational; the panel must be present for attribute-driven pricing to work
- **`Type = Bundle` products** — always need `Allowed` for the component assembly panel

Leave `ConfigureDuringSale` blank only for products with no attributes and no bundle children.

**In `Product2.csv`:** populate the `ConfigureDuringSale` column with `Allowed` for every SKU that appears in `ProductAttributeDefinition.csv`. Cross-check before loading.
