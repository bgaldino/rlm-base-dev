...
|-------------------|-------------|----------|------|------------|
| `tfid-cdo` (`tfid-cdo.json`) | `0TTKX000001N4je` | USA12 | future baseline | **Base CDO license template, no modifications.** The *eventual* `tso=true` baseline goal — reached once all the black-tab mods (see `tfid-cdo-rlm`) are folded into the base license shape template. |
