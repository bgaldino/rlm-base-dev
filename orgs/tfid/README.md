...
| `tfid-pde` (`tfid-pde.json`) | `0TTWt000000rQ2n` | USA1016 | **Partner Developer Edition (PDE) baseline (input)** | TFID snapshot for **Partner Developer Edition** builds (`pde: true`). **Seeded as a verbatim copy of `tfid-cdo-rlm`'s template/instance as a starting point** — replace the `template` ID once a dedicated PDE snapshot is captured. Unlike the external `dev-r1` baseline (a standard DE scratch config), this is the TFID-snapshot path for PDE. |
