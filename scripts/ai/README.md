...
of twenty-one validators a given diff should have run.
...
`tests/test_sfdmu_csv_expectation.py` pins both directions in 212 cases. Two mechanisms make that
...
A full `--all` run is 21 checks in about 17 seconds, of which the branch-scope *suite*
(`tests/test_branch_scope.py`) is 8 —
so the gate costs roughly one branch-scope run more than nothing, and a typical docs-only
selection is a couple of seconds. That timing is measured on a machine where two of the twenty-one
(`docgen_suite`, `harness_suites`) are blocked on optional dependencies and so contribute nothing, which
is worth naming rather than leaving the reader to assume all twenty-one ran: with those installed the
number is higher.

Verified by `tests/test_pr_gate.py` (699 checks, throwaway repos, no network — hermetic for all but
...
pytest-driven check's triggers, each of the twenty-one trigger lists narrowed back off an input its
