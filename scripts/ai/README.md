...
of nineteen validators a given diff should have run.
...
`tests/test_sfdmu_csv_expectation.py` pins both directions in 159 cases. Two mechanisms make that
...
A full `--all` run is 19 checks in about 17 seconds, of which the branch-scope *suite*
(`tests/test_branch_scope.py`) is 8 —
so the gate costs roughly one branch-scope run more than nothing, and a typical docs-only
selection is a couple of seconds. That timing is measured on a machine where two of the nineteen
(`docgen_suite`, `harness_suites`) are blocked on optional dependencies and so contribute nothing, which
is worth naming rather than leaving the reader to assume all nineteen ran: with those installed the
number is higher.

Verified by `tests/test_pr_gate.py` (692 checks, throwaway repos, no network — hermetic for all but
...
pytest-driven check's triggers, each of the nineteen trigger lists narrowed back off an input its
