...
**Unit of Measure (UOM) and UOM Class** are how the system normalizes consumption when usage records arrive in different units. Each Usage Resource is defined with a default UOM and a UOM Class, and the UOM Class groups compatible units together — for example, a **Data** UOM Class can include GB, MB, and TB, and a **Time** UOM Class can include seconds, minutes, and hours. When a usage record arrives in any UOM within the resource's class, the system converts it to that resource's default UOM for summarization and rating. That's what keeps the rated totals consistent when one upstream feed reports in GB and another reports in TB — both convert to the resource's default unit before the rating procedure runs. Pooling multiple grants of the *same* usage resource onto a shared bucket is a separate mechanism — that's the binding policy described above.
