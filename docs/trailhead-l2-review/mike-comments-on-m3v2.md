...
* **NEW #15 (12:39 PM):** "This is really called the Digital Wallet. It shows what resources are accessible, how much is included, and how much is left. It allows customers to drill down into each resource and see the actual debits and credits being made. Can we rewrite it" — Customer-facing UI naming. Initially applied 2026-05-13 as a rename of "Unified Usage Dashboard" → "Digital Wallet". **REVERTED 2026-05-15:** the 262 Help portal grounds this surface as the **Unified Usage Dashboard** (article `ind.um_wallet_management.htm`, titled "Unified Usage Dashboard" — "a single, consolidated source of truth for post-sale usage management"). Mike's "Digital Wallet" reflected the pre-262 name; he was not aware of the 262 rename in the grounding docs (confirmed by Brian, 2026-05-15). The repo now uses **Unified Usage Dashboard** throughout Unit 3, keeping Mike's improved descriptions. The Google Doc never received the c06baa8e rename, so it already reads correctly — the comment thread just needs a reply noting the 262 rename. "Wallet view" remains the correct term for the Experience Cloud surface.

## Thread 0 — UOM vs. UOM Class (resolved 2026-05-15)

Brian's `@maaron` comment "to check on the class conversion" flagged a doubt about the v2 binding-mechanism paragraph claiming that two usage resources sharing a UOM Class pool onto the same Usage Entitlement Bucket. **The v2 framing was wrong; Brian's read was correct.**

262 snapshot confirms (`ind.um_create_unit_of_measure_class.htm`): UOM Class is the unit-conversion mechanism. "For billing purposes, all units of measure in a unit of measure class are converted to the default unit of measure. The conversion factor equates one unit of measure to another." Example given: Weight UOM Class converts kilograms to pounds via a 2.2 factor.

Corrected framing: each Usage Resource is defined with a default UOM and a UOM Class; when a record arrives in any UOM within that class, the system converts it to the resource's default UOM for summarization and rating. UOM Class is *not* a pooling mechanism — pooling multiple grants of the same usage resource onto a shared bucket is the Binding Policy's job (GrantBindingTargetId on the Transaction Usage Entitlement, pointing at Account / Asset / Contract / Custom Object).

Mike's earlier suggestion ("UOM and UOM Class help group like Usage Resources together on the same binding target") used "group" in a loose sense; the actual documented mechanism is conversion, not grouping. The repo paragraph is rewritten to state this accurately and explicitly note that pooling is a separate mechanism. Thread 0 resolved.
