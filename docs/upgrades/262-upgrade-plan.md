...
**Headline finding:** The 260 → 262 schema delta is purely additive — 45 fields added, 0 removed, 0 type changes, and 2 polymorphic-reference targets *expanded* (not narrowed) across the **254 of 263 ERD-tracked platform objects** that were extractable from both scratch orgs (`ent-r1` 260 + `rlm-base__ent-sb0` 262) — so **no SFDMU plan remediation is required**. Nine objects with additive deltas (`EmailTemplate`, `FulfillmentStepDefinition`/`Group`, `FulfillmentTaskAssignmentRule`, `ObjectStateDefinition`, `OmniProcessElement`, `ProductFulfillmentScenario`, `ProductRampSegment`, `ValTfrmGrp`) do appear in existing data plans per `scripts/erd/schema_diff/260-vs-262-diff.md`, but new fields can't break existing CSV imports — they simply remain unpopulated until intentionally adopted.
...
The 9 ERD-tracked objects not present in the scratch org snapshots (`AssetDowntimePeriod`, `AssetOwnerSharingRule`, `AssetShare`, `AssetTag`, `AssetWarranty`, `PricingProcedureResolution`, `ProductPriceHistoryLog`, `ProductPriceRange`, `ProductSellingModelDataTranslation`) require enabled features / shared-object profile permissions not available in the standard RLM scratch profile; their 260↔262 schema was verified separately via Core UDD source review at `gitcore.soma.salesforce.com/core-2206/core-262-public@p4/262-patch` (no changes found). Two original known-issues (#262-3 `RateCard.Status`, #262-4 PUR validation) revised based on schema reality. Full evidence at `.agents/artifacts/262-vs-260-core-schema-research.md` and `.agents/artifacts/262-org-vs-core-cross-validation.md`.
