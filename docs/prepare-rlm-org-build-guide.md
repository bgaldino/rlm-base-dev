...
**Constraints** (Step 24, `constraints` flag) — This is a multi-step process with two tiers of gating. The `constraints` flag enables: loading transaction processing types (also requires `qb`), deploying constraint metadata (classes, triggers, UI components), and applying the context constraint engine node status. The `constraints_data` flag additionally enables: configuring constraint settings via Robot Framework (a UI-only toggle), validating the CML data structure, importing two constraint model datasets (QuantumBitComplete and Server2, both also require `qb`) with polymorphic ID resolution, and activating the expression set versions. This means you can deploy constraint infrastructure without loading constraint model data by setting `constraints: true` and `constraints_data: false`. The CML import is particularly sophisticated — it resolves polymorphic IDs across Product2, ProductClassification, and ProductRelatedComponent records using Salesforce ID prefix detection.
