...
| `configure_core_pricing_recipe_table_mappings` | Ensure core PricingRecipeTableMapping rows exist for NGPDefaultRecipe (RLM_CostBookEntries as a ListPrice table). Uses Tooling API for idempotent create/update without metadata deploy. |
| `configure_pricing_recipe_table_mappings` | Ensure PRM PricingRecipeTableMapping rows exist for NGPDefaultRecipe: RLM_Channel_Program_Level_Partner as PriceAdjustmentMatrix, plus idempotent coverage for the shared RLM_CostBookEntries ListPrice mapping. Uses Tooling API for create/update without metadata deploy. |
