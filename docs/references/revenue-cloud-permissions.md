...
| `RLM_UsageUploader` | `tso`, `quantumbit` | `prepare_tso` step 4 / `prepare_quantumbit` step 9 (running user) — **not** assigned to the salesrep persona | `RLM_UsageUploaderController` Apex class access. Inserts `TransactionJournal` for usage upload, reads usage-related objects. Controller uses `Security.stripInaccessible(CREATABLE, ...)` for DML security hardening. Queries run in **system mode** (WITH USER_MODE / AccessLevel.USER_MODE not available on Summer '26 v67.0 orgs built from this repo's scratch org definitions — feature appears preview/pilot-gated despite existing in other v67.0 classes in this codebase). **Object permissions**: `TransactionJournal` CREATE+READ (CREATE is the only incremental grant; READ overlaps with always-assigned `RLM_USG` PSG); READ on `Asset`, `UsageResource`, `ProductUsageResource`, `UnitOfMeasure`, `AssetRateCardEntry`, `RateAdjustmentByTier`, `TransactionUsageEntitlement`, `UsageEntitlementBucket`, `Account`, `Product2`, `RateCard`, `RateCardEntry` (parent-lookup dependencies; all overlap with `RLM_USG` but retained for standalone functionality). **Field permissions**: comprehensive FLS grants across all queried fields — Asset (AccountId, Product2Id, LifecycleStartDate, LifecycleEndDate), Product2 (UsageModelType), ProductUsageResource (UsageResourceId), UsageResource (DefaultUnitOfMeasureId, UnitOfMeasureClassId), UnitOfMeasure (UnitCode, UnitOfMeasureClassId), AssetRateCardEntry (4 fields), RateCardEntry (3 fields), RateAdjustmentByTier (2 fields), TransactionUsageEntitlement (10 fields), UsageEntitlementBucket (7 fields), plus editable on permissionable `TransactionJournal` create fields. Fifteen fields omitted: six TransactionJournal fields (two required: ActivityDate, Status; four non-permissionable: AccountId, ReferenceRecordId, UsageResourceId, UsageType), and nine required fields on queried objects (AssetRateCardEntry: AssetId, StartDate; ProductUsageResource.Status; UnitOfMeasure.Status; RateAdjustmentByTier: RateCardEntryId, AdjustmentType, AdjustmentValue, LowerBound). **No `pageAccesses` or `ApiEnabled`** — controller does not call `getApiSessionId()`. Before this set existed the controller was granted in **no** permission set anywhere in the repo, so its LWC component worked only for an admin. |
...
| `RLM_UsageUploader` | Usage uploader component (Apex class + system-mode SOQL/DML with Security.stripInaccessible; TransactionJournal CREATE + comprehensive field-level READ grants; no page/ApiEnabled) |
