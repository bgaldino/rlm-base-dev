...
| IsInternal | **Type** boolean **Properties** Create, Defaulted on create, Filter, Group, Sort **Description** Indicates if the procedure output resolution record is created internally by the platform (true) or not (false). The default value is `false`. |
