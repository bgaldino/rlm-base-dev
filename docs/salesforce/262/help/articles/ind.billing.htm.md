...
source_url: https://help.salesforce.com/s/articleView?id=ind.billing.htm&type=5
release: "262"
...
> Breadcrumb: Salesforce Help → Docs → Agentforce Revenue Management → Manage Billing in Agentforce Revenue Management
...
The Billing area contains the following sub-topics. Each will become its own markdown file at `docs/salesforce/262/help/articles/{article_id}.md` once the snapshot is complete.
...
Monitor invoices, credit memos, and invoice schedules with ease. Manage billing transactions effectively with timely insights into revenue performance and operational health.

---

*Source-of-truth note: this article was captured via the release-enablement skill's Chrome MCP shadow-DOM walker pattern. The body text is verbatim from the Salesforce Help portal as of the `fetched_at` date in the frontmatter. Article IDs for child articles will be populated when the full Billing-area snapshot completes.*
