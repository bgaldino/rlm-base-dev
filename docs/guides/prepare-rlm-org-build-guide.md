...
> Last Updated: April 2026 | Salesforce Release 260 (Spring '26) | API v66.0
...
**What happens:** Payments infrastructure is set up, core Salesforce metadata is deployed, price adjustment schedules are activated, and QuantumBit-specific metadata and permissions are applied.
...
**Payments preparation** (Step 4, conditional on `payments` flag) — Creates and deploys the Experience Cloud site metadata used for payments webhook endpoints. This must happen early because the site can take time to provision and later steps depend on it.
...
**QuantumBit preparation** (Step 7) — This step always runs, but if `quantumbit` is off every task inside it is skipped. When enabled, it deploys QuantumBit-specific metadata (UI themes, utility flows, billing flexipages), sets up approval workflows, assigns the QuantumBit permission set, and enables CALM (Customer Asset Lifecycle Management) delete permissions. Two separate flags control QuantumBit content: `quantumbit` controls this metadata deployment, while the product/pricing data loads later in the flow are controlled independently by the `qb` flag.
...
### Phase 4: Business Process Configuration (Steps 10–16)
...
**Analytics** (Step 14, `analytics` flag) — Enables Data Sync/Connections replication, which allows Revenue Cloud data to flow to CRM Analytics for reporting.

**CLM** (Step 15, `clm` + `clm_data` flags) — Loads Contract Lifecycle Management reference data including contract templates, clause libraries, and related configuration.

**Rating** (Step 16, `rating` and `rates` flags) — Loads usage rating design-time data when `rating` is enabled. Rate card loading and the final activation steps additionally require `rates` to be on. Rating data is loaded in two passes due to self-referential relationships between Product Usage Resources (PURs), Product Usage Resource Periods (PURPs), and Product Usage Groups (PUGs). Rate card data is loaded separately when `rates` is also enabled. Activation happens in two separate steps: a 7-step Apex script activates PURs, PURPs, and PUGs in the platform-required order, then a second Apex script activates the rate card entries.

---

### Phase 5: Expression Sets and Permissions (Steps 17–21)
...
**Expression set activation** (Step 17) — Re-deploys expression sets with active status using XPath transformation of the metadata XML. This is a deliberate two-pass approach: deploy as draft first to avoid validation errors against missing data, then activate once all data is in place.

**TSO preparation** (Step 18, `tso` flag) — Assigns additional permission set licenses, permission set groups, and metadata bundles specific to Trialforce Source Orgs. TSOs have a superset of permissions because they're used to generate trial orgs that need to work out of the box.

**Procedure plans** (Step 19, `procedureplans` flag) — Creates Procedure Plan Definitions and their associated sections and options via the Connect REST API and SFDMU data loading. Procedure plans define the step-by-step flows for quote pricing and other revenue processes.

**PRM** (Step 20, `prm` flag) — Creates the Partner Central community, publishes it, and extends the Sales Transaction Context with partner account attributes — all of which happen whenever `prm` is on. Loading the QuantumBit PRM product data additionally requires `qb`; if you're building a Q3-only org, the community is still created and published but that data load is skipped. Two sub-steps are Trialforce Source Org-specific and only run in TSO builds: deploying the full Experience Bundle and assigning the `RLM_PRM` permission set. Sharing rules deployment is its own separate toggle (`sharingsettings`).

**Agentforce agents** (Step 21, `agents` flag) — Deploys Agentforce AI agent configurations, settings, and assigns the quoting agent permission set.

---

### Phase 6: Constraints and Guided Selling (Steps 22–23)
...
### Phase 7: Final Configuration (Steps 24–30)

**What happens:** Revenue settings are configured via Robot Framework, pricing discovery is reconfigured for scratch orgs, the ramp-builder package is deployed, optional UX assembly runs, scratch seed records are inserted, decision tables are refreshed, and the run is stamped with git metadata.
...
**Ramp builder deployment** (Step 26, `ramps` flag) — Deploys ramp-builder metadata (including Quote/Quote Line Group assets and supporting automation) and applies context mappings needed for ramp scheduling.

**Dynamic UX assembly** (Step 27, `ux` flag) — Assembles and deploys UX assets generated from `templates/` for page/layout/profile composition in the target org.

**Scratch seed data** (Step 28, scratch orgs only, not TSO) — Inserts baseline Account/Contact/BillingAccount records used by later automation and demo paths in newly created scratch orgs.
...
**Git stamp** (Step 30) — Stamps the org with the current repository commit metadata for traceability and troubleshooting.
