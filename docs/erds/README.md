...
3. Run `python scripts/erd/cleanup_orphan_erd_fields.py --orgs <alias>,<alias2> --dry-run` to identify candidates for cleanup. Pass **two distinct orgs of the target release and same shape** so no orphan verdict rests on a single org — the 264 pass used `rlm-base__264merged,rlm-base__264fresh`
