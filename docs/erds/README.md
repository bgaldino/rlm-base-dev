...
> The 262 baseline was 33 objects with field gaps and 822 ERD-side fields. Both collapsed at 264 because the refresh patched in every field the org had, so the only ERD-side residue left is the deliberately carried-forward set. Quote the report rather than these numbers if they disagree — `validation-report.md` is generated, this line is not.
