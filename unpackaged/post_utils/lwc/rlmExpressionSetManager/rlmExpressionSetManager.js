...
import getActiveJobStatus from '@salesforce/apex/RLM_ExpressionSetManagerController.getActiveJobStatus';
...
const COLUMNS = [
    ...
    { label: 'Type', fieldName: 'usageType', type: 'text', sortable: true, initialWidth: 130 },
    { label: 'Status', fieldName: 'statusText', type: 'text', sortable: true, initialWidth: 110 },
    {
...
export default class RlmExpressionSetManager extends LightningElement {
    ...
    async connectedCallback() {
        try {
            ...
            if (status === 'running') {
                this.isActing = true;
                this.showToast('In Progress', 'A previous operation is still running. Waiting for completion...', 'info');
                this.pollJobCompletion();
            } else if (status.startsWith('failed:')) {
                this.previousError = status.substring(7);
            }
        } catch (error) {
            // ignore — surface only if user attempts an action
        }
    }
    ...
    async handleDeactivate() {
        ...
        if (active.length === 0) {
            this.showToast('Info', `All ${selectedRows.length} selected Expression Set(s) are already inactive.`, 'info');
            return;
        }
            ...
            this.pollStatus(getDeactivateStatus, this.selectedContextId, defIds, 0, (ids) => {
                this.expressionSets = this.expressionSets.map(es =>
                    ids.has(es.definitionId)
                        ? { ...es, hasActiveVersion: false, statusText: 'Inactive' }
                        : es
                );
    ...
    pollJobCompletion() {
        ...
        this._pollTimer = setInterval(async () => {
            try {
                ...
                if (status === 'none') {
                    this.stopPolling();
                    this.showToast('Complete', 'Previous operation finished.', 'success');
