trigger RLM_QuoteItemTrigger on QuoteLineItem(before insert) {
    ...
    if (!quoteActionIds.isEmpty()) {
        ...
        for (QuoteAction qAction : [
            ...
            FROM QuoteAction
            WHERE SourceAssetId != NULL AND Id IN :quoteActionIds
        ]) {
        ...
        for (AssetActionSource actionSource : assetActionSources) {
            if (
                !assetIdToStatus.containsKey(actionSource.AssetAction.AssetId) &&
                actionSource.RLM_ConstraintEngineNodeStatus__c != null
            ) {
                assetIdToStatus.put(actionSource.AssetAction.AssetId, actionSource.RLM_ConstraintEngineNodeStatus__c);
            }
        }
