...
> This file guides GitHub Copilot when reviewing or working with this repository.

## Project Overview

**Revenue Cloud Base Foundations** automates creation and configuration of Salesforce environments for Revenue Lifecycle Management (RLM). This branch (`262-test`) targets Salesforce Release 262 (Summer '26, API v67.0). `main` remains on Release 260 (Spring '26) until Summer '26 is GA.
...
> The three bugs below are the authoritative reference. `docs/references/sfdmu-composite-key-optimizations.md` covers dataset-level migration changes (which externalIds were simplified and why) but does not document these bugs individually.
...
See `## macOS Environment Setup (Homebrew + pyenv + nvm)` section in README.md for the authoritative step-by-step guide. Key notes:
